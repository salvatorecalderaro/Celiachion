...
def prepare_dataset(dataset_file, dataset_name):
    ...
    train.to_csv(
        neuro_networks_path + "ethalons_" + dataset_name + ".csv", index=False
    )
    test.to_csv(
        neuro_networks_path + "candidates_" + dataset_name + ".csv",
        index=False,
...
while c != 0:
    ...
    if c > 0 and c < 5:
        num_params = -1
        num_neurons_first_hidden_layer = int(
            input("Number of neurons in first hidden layer: ")
        )
        num_neurons_second_hidden_layer = int(
            input("Number of neurons in second hidden layer: ")
        )
        if c == 1:
